      ...
      <div className="space-y-4 sm:space-y-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 flex items-center gap-2 sm:gap-3">
            ...
            {t('client.messages_title')}
          </h1>
          <p className="text-sm text-slate-500 mt-1">{t('client.messages_subtitle')}</p>
        </div>

        <div className="grid lg:grid-cols-[350px_1fr] gap-4 sm:gap-6 min-h-[400px] sm:min-h-[600px]">
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <h2 className="font-bold text-slate-900 text-sm">{t('client.messages_select_package')}</h2>
            </div>
            <div className="overflow-y-auto max-h-[540px]">
              {loading ? (
                <div className="p-6 text-center text-slate-400 text-sm">{t('client.messages_loading')}</div>
              ) : packages.length === 0 ? (
                <div className="p-6 text-center text-slate-400 text-sm">{t('client.messages_empty')}</div>
              ) : (
                packages.map((pkg) => (
                  <button
                    key={pkg.id}
            ...
            {!selectedPkg ? (
              <div className="flex-1 flex items-center justify-center text-slate-400 bg-white rounded-xl border border-slate-200 shadow-sm">
                <div className="text-center">
                  ...
                  <p className="text-sm">{t('client.messages_select_hint')}</p>
                </div>
              </div>
            ) : (
              <MessagingPanel
